...
public interface VersRepository extends PagingAndSortingRepository<Vers, Long> {
    ...
    @Query(value = "SELECT v.id, v.versid, v.rank, v.title, v.name, v.location, v.versLines FROM vers v, song s where s.songid=:songid AND s.songid=v.fk_song order by v.rank", nativeQuery = true)
